...
object MyModule {
  ...
  def fib(n: Int): Int = {
    ...
    def go(i: Int, prev: Int, acc: Int): Int =
      if (i < n)
        if (i == 0) go(i + 1, 0, 0)
        else if (i == 1) go(i + 1, 0, 1)
        else if (i == 2) go(i + 1, 1, 1)
        else
          go(i + 1, acc, prev + acc)
      else
        acc
...
object PolymorphicFunctions {
  ...
  def isSorted[A](as: Array[A], gt: (A,A) => Boolean): Boolean = {
    @annotation.tailrec
    def loop(n: Int, prev: A, cur: A): Boolean = {
      if (n >= as.length) gt(cur, prev)
      else if (!gt(cur, prev)) false
      else loop(n + 1, cur, as(n))
    }
    if (as.length <= 1) true
    else loop(1, as(0), as(1))
  }
